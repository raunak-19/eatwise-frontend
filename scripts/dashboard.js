...
document.addEventListener("DOMContentLoaded", () => {
  ...
  sendBtn.addEventListener("click", () => {
    ...
    fetch("https://eatwise-backend-5ykd.onrender.com/ask-ai", {
      method: "POST",
...
If context is relevant, use it.
  ...
  function handleImageFile(file) {
    ...
    fetch("https://eatwise-backend-5ykd.onrender.com/analyze-image", {
      method: "POST",
      body: formData
    })
      .then(res => res.json())
  ...
  if (autoFillBtn) {
    autoFillBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const response = await fetch('https://eatwise-backend-5ykd.onrender.com/ask-ai', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
